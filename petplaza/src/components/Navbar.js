...
const Navbar = ({ user, onLogout }) => {
      ...
      const checkInventory = async () => {
        ...
        const hasChanges =
          JSON.stringify(data.expired) !==
            JSON.stringify(lastDataRef.current?.expired) ||
          JSON.stringify(data.expiringSoon) !==
            JSON.stringify(lastDataRef.current?.expiringSoon) ||
          JSON.stringify(data.lowStock) !==
            JSON.stringify(lastDataRef.current?.lowStock);
  ...
  else if (location.pathname === "/Inventory" && invAlerts?.total > 0)
  ...
  return (
    <div className="navbar-wrapper">
        ...
        <div className="navbar-icon-wrapper" ref={dropdownRef}>
          ...
          >
            ...
            {(appointments.length > 0 ||
              alertaCAI ||
              (location.pathname === "/Inventory" && invAlerts?.total > 0)) && (
                    ...
                    <>
                      {invAlerts.expired.length > 0 && (
                        <p className="text-red-500 font-semibold">
                          🔴 {invAlerts.expired.length} productos vencidos
                        </p>
                      )}
                      {invAlerts.expiringSoon.length > 0 && (
                        <p className="text-yellow-500 font-semibold">
                          🟡 {invAlerts.expiringSoon.length} por vencer en 30
                          días
                        </p>
                      )}
                      {invAlerts.lowStock.length > 0 && (
                        <p className="text-orange-500 font-semibold">
                          🟠 {invAlerts.lowStock.length} con stock bajo
                        </p>
                      )}
                    </>
                  )}
      ...
      {showModal && (
        <div
          className={`navbar-logout-modal-overlay ${
            isClosing ? "fade-out" : ""
          }`}
          onClick={closeModal}
        >
          <div
            className={`navbar-logout-modal-content ${
              isClosing ? "fade-out" : ""
            }`}
          ...
          >
            ...
            <h3 className="navbar-logout-modal-title">
